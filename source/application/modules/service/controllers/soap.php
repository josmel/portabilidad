<?php
     ...
     if ($_SERVER['REQUEST_METHOD']=='GET') {
           if($_GET["wsdl"] === "" && isset($_GET["wsdl"]) && empty($_GET["wsdl"]))
            ...
            $str .= '     <soap12:address location="https://10.209.43.2/MOCHEWebApp/services/MOCHEWebService?wsdl"/>' . "\n";
            $str .= '   </wsdl:port>' . "\n";
            $str .= ' </wsdl:service>' . "\n";
           ...
           function receiveMessage($a) 
          {
            ...
            $result =  $ReceiveMessage->_receiveMessages($a);
                ...
                if ($enlace) {
                    $texto = print_r($a, true);
                    $archivo = APPLICATION_PATH . '/../data/tramas_recibido/datos_recibidos_abdcp_producion_' . date('Ymdhis') . ".txt";
                    $f = fopen($archivo, "x");
                    fwrite($f, $texto);
                    fclose($f);
                }

                return array("response" => 'ack');
            }
            ...
            $server = new SoapServer("https://10.209.43.2/MOCHEWebApp/services/MOCHEWebService?wsdl");
            $server->addFunction("receiveMessage");
